...
echo  "$($PSQL "SELECT CAST(AVG(winner_goals) AS numeric(10,2)) FROM games")"
...
echo "$($PSQL "SELECT t.name FROM teams t where exists(select 1 from games g where (g.winner_id=t.team_id or g.opponent_id=t.team_id) and g.year='2014' and g.round='Eighth-Final') order by t.name")"
